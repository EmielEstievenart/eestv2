#include "find_thursday.hpp"

#include "day_off_planning_validator.hpp"
#include "double_day_planning_validator.hpp"
#include "find_friday.hpp"
#include "weekday_shifts.hpp"

#include <cstddef>

void find_possible_thursdays(WeekPlanning planning, DaysOfTheWeek search_until, const SearchResultCallback& on_found, SearchContext* context)
{
    DoubleDayPlanningValidator validator;
    auto thursday_planning = planning.thursday.value_or(OneDayPlanning<WeekdayShiftCode>(get_weekday_required_shifts()));
    if (!DayOffPlanningValidator::apply_mandatory_days_off(planning, thursday_planning, DaysOfTheWeek::thursday, DayOffPlanningValidator::default_max_consecutive_days, get_off_shift))
    {
        return;
    }

    auto nr_of_combinations = thursday_planning.get_nr_of_combinations();
    for (auto index = 0; index < nr_of_combinations; index++)
    {
        if (search_cancel_requested(context))
        {
            report_search_progress(context, DaysOfTheWeek::thursday, search_until, index, nr_of_combinations, true);
            return;
        }

        report_search_progress(context, DaysOfTheWeek::thursday, search_until, index, nr_of_combinations);

        auto thursday_candidate = thursday_planning.get_set(index);
        bool is_valid           = true;

        for (std::size_t person = 0; person < thursday_candidate.size(); ++person)
        {
            const auto previous_shift = planning.getPreviousWeekdayShift(static_cast<int>(person), DaysOfTheWeek::thursday);
            if (previous_shift.has_value() && !validator.is_valid(*previous_shift, thursday_candidate.get(person).get_code()))
            {
                is_valid = false;
                break;
            }

            const auto next_shift = planning.getNextWeekdayShift(static_cast<int>(person), DaysOfTheWeek::thursday);
            if (next_shift.has_value() && !validator.is_valid(thursday_candidate.get(person).get_code(), *next_shift))
            {
                is_valid = false;
                break;
            }
        }

        if (!is_valid)
        {
            continue;
        }

        auto candidate_planning = planning;
        candidate_planning.thursday.emplace(thursday_candidate);
        mark_search_day_valid(context, DaysOfTheWeek::thursday);
        report_search_progress(context, DaysOfTheWeek::thursday, search_until, index, nr_of_combinations, false, &candidate_planning);

        if (search_until == DaysOfTheWeek::thursday)
        {
            if (context != nullptr)
            {
                ++context->found_count;
                report_search_progress(context, DaysOfTheWeek::thursday, search_until, index, nr_of_combinations, false, &candidate_planning);
            }
            on_found(candidate_planning);
            continue;
        }

        if (search_cancel_requested(context))
        {
            report_search_progress(context, DaysOfTheWeek::thursday, search_until, index, nr_of_combinations, true);
            return;
        }

        find_possible_fridays(candidate_planning, search_until, on_found, context);
    }
}
