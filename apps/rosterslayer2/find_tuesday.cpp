...
void find_possible_tuesdays(WeekPlanning planning, DaysOfTheWeek search_until, const SearchResultCallback& on_found, SearchContext* context)
{
    ...
    if (!DayOffPlanningValidator::apply_mandatory_days_off(planning, tuesday_planning, DaysOfTheWeek::tuesday, DayOffPlanningValidator::default_max_consecutive_days, get_off_shift))
    {
        return;
    }
