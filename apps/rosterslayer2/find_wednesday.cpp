...
void find_possible_wednesdays(WeekPlanning planning, DaysOfTheWeek search_until, const SearchResultCallback& on_found, SearchContext* context)
{
    ...
    if (!DayOffPlanningValidator::apply_mandatory_days_off(planning, wednesday_planning, DaysOfTheWeek::wednesday, DayOffPlanningValidator::default_max_consecutive_days, get_off_shift))
    {
        return;
    }
